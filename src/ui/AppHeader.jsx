...
export default function AppHeader({ screen, solveReady, labels, onGo, onToggleLabels, onReset }) {
  return (
    <div className="cw-header" style={{ height: 52, flex: 'none', display: 'flex', alignItems: 'center', gap: 22, padding: '0 18px', background: BG_PANEL, borderBottom: `1px solid ${LINE}` }}>
        ...
        {TABS.map((tab, i) => {
          ...
          return (
            <React.Fragment key={tab.screen}>
              {i > 0 && <div className="cw-hide-sm" style={{ width: 14, height: 1, background: 'rgba(255,255,255,0.14)' }} />}
              <Btn
              ...
              >
                <span className="cw-hide-sm" style={{ ...mono, fontSize: 10, opacity: 0.6 }}>{tab.num}</span>{tab.title}
              </Btn>
            </React.Fragment>
          );
        })}
      ...
      >
        <span style={{ ...mono, fontSize: 11 }}>Aa</span><span className="cw-hide-sm">Color labels</span>
      </Btn>
      <ConfirmBtn
        onConfirm={onReset}
