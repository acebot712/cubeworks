...
\usepackage{tmlr}
% SUBMISSION-READY AS IS. TMLR review is double-blind: without the [accepted]
% option the author block below is ignored and the title page reads "Anonymous
% authors / Paper under double-blind review". Add [accepted] only for the
% camera-ready version, which is also the only mode where \openreview is printed.
...
and So Do Pattern Databases, by the Same Two-Moment Law}

% AUTHOR BLOCK IS COMMENTED OUT ON PURPOSE. tmlr.sty ignores it without the
% [accepted] option, so the compiled PDF was anonymous either way -- but the
% SOURCE is not the PDF. Reviewers who are sent the project archive, or anyone
% the Overleaf project is shared with, would have read the name straight out of
% this file. Restore it for the camera-ready, together with [accepted] on line 2.
...
a single Kendall $\tau$ against exact distance-to-goal, pooled over every pair of
states. We measure it \emph{stratified by distance} instead, against exhaustive
ground truth, on Rubik's-cube sub-problems spanning $5.5\times10^{2}$ to
$3.1\times10^{23}$ states.

Call the set of states at exact distance $d$ from the goal the \emph{shell} at
$d$. A search compares adjacent shells, and we find that a heuristic's accuracy
at that comparison is $\Phi(\Delta/(\sigma\sqrt{2}))$ --- where $\Delta$ is the
difference in mean heuristic value between the two shells, $\sigma$ the spread of
values within them, and $\Phi$ the standard normal distribution function --- with
no fitted parameters. Estimating the moments and measuring accuracy on disjoint
halves of each shell gives a mean absolute error of $0.012$ over 252 shell
observations, against an estimator noise floor of $0.002$. It holds on a
sub-problem 300 times larger whose ground truth comes from a different apparatus,
and on heuristics trained under three further objectives. Injecting noise of known
tail weight degrades it monotonically, and Cauchy noise, which has no finite
variance, breaks it by a factor of 36 --- so the normal form, not mere
monotonicity, carries the prediction.

Three consequences follow. Ordering quality is invariant to rescaling. Decay with
distance occurs exactly when $\sigma$ grows, which is why it is \emph{not}
particular to learned heuristics: pattern databases on the same states decay as
much, and matched on strength the two classes are indistinguishable. And $\sigma$
is the one term an objective can act on --- but every objective we built to act on
it produced a worse heuristic, monotonically in the penalty weight.

We also show that $\tau$-b rewards a heuristic for being coarse-valued, by enough
($0.084$) to reverse the learned-versus-classical comparisons this statistic is
used to make. The identity is classical; the measurement against exact ground
truth is our contribution. We report three claims made by earlier versions of this
work that our own controls subsequently refuted.
\end{abstract}
...
increasing length --- the depth curriculum we adopt and claim nothing for.
...
\begin{table}[t]\centering\input{tables/exact}
...
\begin{table}[t]\centering\input{tables/matched}
...
\begin{table}[t]\centering\input{tables/strength}
...
\begin{table}[t]\centering\input{tables/law}
...
\begin{table}[t]\centering\input{tables/noise}
...
\begin{table}[t]\centering\input{tables/losses}
...
a scale-free ranking loss can never satisfy it --- using it would have pinned
...
the baseline's $0.219$ --- while solving 44\% of scrambles against the baseline's
...
interventions is exhausted --- targeting the numerator, or the variance of the
...
\begin{table}[t]\centering\input{tables/depth}
...
separate matter and is unaffected --- every run there reached the curriculum
ceiling, so its $\pm23.5$ percentage points at $k=10$ is genuine variation rather
than this deadlock.

\begin{table}[t]\centering\input{tables/ladder}
...
\paragraph{The law is an approximation, and we can see the gap.} Split-sample
mean absolute error is $0.012$ against an estimator noise floor of $0.002$. The
residual is six times the noise, so Equation~\ref{eq:law} is demonstrably not
exact on this data --- real value distributions are not quite normal, and the
deviation is largest for the small-integer pattern databases. We report the
approximation and its error rather than claiming a law that holds exactly.
...
\paragraph{One domain.} All results are Rubik's-cube sub-problems. Whether the
distance-dependent profile appears in classical planning, where learned heuristics
are also trained from goal-anchored data, is untested here.
...
runtimes, and the seeds used are documented in the supplementary material,
including a list of superseded results (retained rather than deleted) and of
measurements the apparatus could not obtain.
